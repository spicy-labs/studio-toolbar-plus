...
export function layoutMappingValidation(
  layoutMap: LayoutMap,
  doc: Doc,
): {
  ...
  const existingVariableIds = new Set(
    doc.variables.map((variable) => variable.id),
  );
  ...
  cleanLayoutMap.variables = cleanLayoutMap.variables.filter(
    (imageVariable) => {
      ...
      processImageVariableDependentGroups(
        imageVariable,
        existingVariableIds,
        report,
      );
...
function processImageVariableDependentGroups(
  ...
  imageVariable.dependentGroup = imageVariable.dependentGroup.filter(
    (group, groupIndex) => {
      ...
      group.variableValue = group.variableValue.filter((value) => {
        ...
        const valueExists = variableValue.id
          ? existingVariableIds.has(variableValue.id)
          : true;

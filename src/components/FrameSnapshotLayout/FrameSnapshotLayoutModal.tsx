...
export function FrameSnapshotLayoutModal({
  ...
  const [frameLayoutMaps, setFrameLayoutMaps] = useState<
    EnhancedFrameLayoutMap[]
  >([]);
  ...
  const tableData = useMemo(() => {
    return frameLayoutMaps.map((frameLayoutMap, layoutIndex) => {
      ...
      let snapshots = frameLayoutMap.frameSnapshots.map(
  ...
  const handleEditCell = useCallback(
    (layoutId: string, key: string, value: string | number) => {
      if (!key) return;
      ...
      setFrameLayoutMaps((prev) => {
        const next = [...prev];
        const frameSnapshot = next
          .find((map) => map.layoutId === layoutId)
          ?.frameSnapshots.find((s) => s.uniqueId === uniqueId);

        if (!frameSnapshot) return prev;

        if (field === "imageName") {
          // Check if name already exist on a frameSnapshot
          const nameExists = next.some((map) =>
            map.frameSnapshots.some(
              (s) => s.imageName === value && s.uniqueId !== uniqueId,
            ),
          );
          ...
          frameSnapshot[field as "x" | "y" | "width" | "height"] =
            Number(value);
        }
  ...
  const loadFrameLayouts = async () => {
    ...
    try {
      ...
      if (!frameLayoutsResult.isOk()) {
        raiseError(
          new Error(
            frameLayoutsResult.error?.message || "Failed to load frame layouts",
          ),
        );
        return;
      }
      ...
      const enhancedFrameLayoutMaps = frameLayoutsResult.value.map(
        (frameLayoutMap) => ({
          ...frameLayoutMap,
          ...
          layoutName:
            layoutMap.get(frameLayoutMap.layoutId) || "Unknown Layout",
        }),
  ...
  const handleRemoveFrameLayout = async (
    layoutId: string,
    uniqueId: string,
  ) => {
      ...
      setFrameLayoutMaps((prev) => {
        ...
        for (let i = 0; i < next.length; i++) {
          ...
          if (layout.layoutId === layoutId) {
            ...
            const snapshotIndex = layout.frameSnapshots.findIndex(
  ...
  const handleUpdateActions = async () => {
    ...
    try {
      ...
      if (!saveResult.isOk()) {
        raiseError(
          new Error(
            saveResult.error?.message || "Failed to save frame layout maps",
          ),
        );
        return;
      }
      ...
      const result = await saveImageSizingMappingToAction(
        cleanupFrameLayoutMaps(),
      );
    ...
    >
      ...
      >
        ...
        <Group justify="flex-end" mt="md">
          ...
          <Button onClick={handleCleanupAndClose} disabled={isRemoving}>
            Close
          </Button>
        </Group>
      </div>

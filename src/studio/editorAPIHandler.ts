...
import type { PerAssetCrop, AssetCrop } from '../studio-adapter/manualCropTypes';

type EditorResponseWithoutParsedData<T> = Omit<EditorResponse<T>, 'parsedData'>;

function simplifyEditorResponse<T>(editorResponse: EditorResponse<T>): EditorResponseWithoutParsedData<T> {
    return {
        ...editorResponse,
    }
}

async function handleApiCall<T, Args extends any[] = any[]>(apiCall: (...args: Args) => Promise<EditorResponse<string>>, ...args: Args) {
    const result = await Result.try(async () => simplifyEditorResponse(await apiCall(...args)));
...
export async function setPerAssetCrop({ studio, perAssetCrop, layoutId, frameId, remoteConnectorId, assetId }: SetPerAssetCropParams) {
    const api = await studio.editorAPI;
    return handleApiCall(api.setPerAssetCrop, layoutId, frameId, remoteConnectorId, assetId, JSON.stringify(perAssetCrop));
}